...
response = requests.get(BASE + f'get-oferta-cadeira/{str(cadeira.get("id"))}')
print('D', response.json())
print()

data = {'cadeira': str(cadeira.get('id')), 'horario': {'sex': [15, 16]}, 'centro_universitario': 'CIn', 'professor': 2, 'periodo': '2023.1'}
payload = json.dumps(data)
response = requests.post(BASE + 'cadastrar-oferta-cadeira', headers=headers, data=payload)
oferta_cadeira = response.json()
print('C', oferta_cadeira)
